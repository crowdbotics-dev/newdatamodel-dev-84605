      ...
      <ScrollView>
        <View style={styles.sectionContainer}>
          ...
          <View style={styles.sliderContainer}>
            <Text>$80</Text>
            <Slider style={styles.slider} minimumValue={80} maximumValue={500} step={1} value={sliderValue} onValueChange={value => setSliderValue(value)} minimumTrackTintColor="#40BA00" maximumTrackTintColor="#eee" thumbTintColor="red" thumbStyle={styles.thumb} />
            <Text>$500</Text>
          </View>
        </View>
        <View style={styles.sectionContainer}>
          <Text style={styles.sectionTitle}>Types of Place</Text>
          {typesOfPlace.map((item, index) => <View style={styles.placeItem} key={index}>
              <View>
                <Text>{item.name}</Text>
                <Text style={styles.grey}>{item.description}</Text>
              </View>
              <Checkbox value={selected.includes(item)} setValue={() => handleSelect(item)} />
            </View>)}
        </View>
        <View style={styles.sectionContainer}>
          <Text style={styles.sectionTitle}>Rooms and beds</Text>
          <Counter name="Beds" value={numBeds} onValueChange={x => setNumBeds(x)} />
          <Counter name="Bedrooms" value={numBedrooms} onValueChange={x => setNumBedrooms(x)} />
          <Counter name="Bathrooms" value={numBaths} onValueChange={x => setNumBaths(x)} />
        </View>
        <View style={styles.sectionContainer}>
          <Text style={styles.sectionTitle}>Amenities</Text>
          {amenities.map((item, index) => <View key={index}>
              <Text>{item.name}</Text>
              {item.list.map((i, index1) => <View style={styles.amenityItem} key={index1}>
                  <Text>{i}</Text>
                  <Checkbox value={selected.includes(i)} setValue={() => handleSelect(i)} />
                </View>)}
            </View>)}
        </View>
        <View style={styles.sectionContainer}>
          <Text style={styles.sectionTitle}>Property Type</Text>
          {propertyTypes.map((item, index) => <View key={index}>
              <Text>{item.name}</Text>
              {item.list.map((i, index1) => <View style={styles.amenityItem} key={index1}>
                  <Text>{i}</Text>
                  <Checkbox value={selected.includes(i)} setValue={() => handleSelect(i)} />
        ...
        <View style={styles.footer}>
          <Button buttonText="Clear all" color="white" textColor="#000" hideShadow={true} onPress={() => setSelected([])} />
  ...
  return <View style={counterStyles.container}>
      ...
      <View style={counterStyles.counterContainer}>
        <Pressable onPress={() => props.onValueChange(decrement(props.value))}>
          <Image source={require("./assets/decrementIcon.png")} style={[counterStyles.icon, props.value === 0 && counterStyles.disabled]} />
        </Pressable>
        <Text>{props.value}</Text>
        <Pressable onPress={() => props.onValueChange(increment(props.value))}>
          <Image source={require("./assets/incrementIcon.png")} style={counterStyles.icon} />
        </Pressable>
      </View>
    </View>;
};
